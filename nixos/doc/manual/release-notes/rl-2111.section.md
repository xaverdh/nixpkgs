...
- The linux kernel package infrastructure was moved out of `all-packages.nix`, and restructured. Linux related functions and attributes now live under the `pkgs.linuxKernel` attribute set.
  The versioned `linuxPackages_*` package sets and kernels from `pkgs` are now considered deprecated, and kept for backwards compatibility only. If you want to pin the kernel package to a specific version, use one of the `linuxKernel.packages.*` attributes.
  This refactoring allowed us to add a new function `linuxKernel.latestPackageWithModules` that you can use to obtain the lastest kernel in `nixpkgs`, where a given list of kernel modules is not broken.
